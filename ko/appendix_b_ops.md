...
그런데 파일럿 프로젝트에서는 도로망 위를 실제로 달린 결과가 필요합니다. 내장 엔진은 직선거리 근사라서, 그때는 서버가 필요합니다. 이 부록은 서버를 띄우는 방법과 수업에서 운영할 때 걸리는 것들을 정리합니다.
...
`mode` 가 `live` 로 나오면 서버에 붙은 것입니다. `local` 이면 서버 없이 내장 엔진으로 도는 것입니다(녹화본이 있는 요청은 녹화본).
